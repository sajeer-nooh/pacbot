...
class SystemInstallValidation(SystemValidation):
    ...
    def validate(self):
        ...
        if(Settings.get('MINIMUM_RAM',None) and Settings.get('MINIMUM_STORAGE',None)):
            status = self.validate_system_config()
            self.show_step_inner_messaage(K.SYSTEM_CONFIG_CHECK_STARTED, status, self.error_message)
            if status != K.VALID:
                return False
